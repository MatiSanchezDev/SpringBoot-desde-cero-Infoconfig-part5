# Ferretería Antillana — API REST con Spring Boot

Proyecto desarrollado como parte del curso **"Spring Boot desde Cero"** de Infoconfig (Parte 5).
Es una API REST completa para gestionar el inventario de artículos de una ferretería, con frontend integrado servido por el mismo servidor.
...
│   │   │   ├── Service/
│   │   │   │   └── ArticuloService.java          ← Lógica de negocio
│   │   │   ├── Repository/
│   │   │   │   └── Repo_Articulos.java           ← Acceso a BD con JPA
│   │   │   ├── Entity/
│   │   │   │   └── Articulos.java                ← Entidad JPA (tabla en BD)
│   │   │   ├── DTOs/
...
La tabla necesaria en MySQL es:
...
| Método HTTP | URL                         | Descripción                        | Body requerido                   |
|-------------|-----------------------------|------------------------------------|----------------------------------|
| `GET`       | `/api/articulos`            | Obtiene todos los artículos        | —                                |
| `GET`       | `/api/articulos/{id}`       | Obtiene un artículo por su ID      | —                                |
| `POST`      | `/api/articulos`            | Crea un nuevo artículo             | `{ nombre, precio, existencia }` |
| `PUT`       | `/api/articulos/{id}`       | Actualiza un artículo existente    | `{ nombre, precio, existencia }` |
